---
title: "Appendix PD vs Controls, RNA-seq"
output:
  html_document:
    toc: true
    toc_depth: 2
...
params <- list(
  ...
  clustering    = list(
    groups      = c(Diagnosis="Diagnosis", Gender="Sex", Plate="substr(Sample_ID, 1, 1)", RIN = "RIN < median(RIN)"),
    addClustersToModel = 0
  ),
  normalization = list(
    normalize   = FALSE,
    plotColor   = c("Diagnosis")
  ),
  imputation    = list(
    imputeBetas = FALSE,
    imputeCounts= FALSE
...
rnaseq <- function() {
  ...
  data <- list()
  path <- "../input/data cohort4_PD_Appendix_Peipei/RNA-Seq_VAI/totalRNA_PD_Control/"

  # read the sample key
  data$sample <- 
    fread(file.path(path, "totalRNA_PD_Control_covar_sva.txt")) %>%
...
# touch()
data <- cache(foo=rnaseq, fname="data.RDS", verbose=TRUE)
...
```{r, include=TRUE}
data$modelData %>% 
  ggplot(aes(Diagnosis, Age, color=Sex)) + 
  geom_boxplot() +
  theme_bw(base_size=10)
```
